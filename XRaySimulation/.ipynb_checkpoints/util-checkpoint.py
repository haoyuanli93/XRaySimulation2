...
def rot_mat_in_yz_plane(theta):
    """
    Get a rotation matrix 3x3 for rotation around x rotation_axis
    in the yz plane
...
def get_k_mesh_3d(number_x, number_y, number_z, delta_e_x, delta_e_y, delta_e_z):
    ...
    # Get the spatial mesh along x rotation_axis
    ...
    # Get the spatial mesh along y rotation_axis
...
def get_mirror_kout(kin,
    ...
    # Get the projection of the kin_array along the direction of normal
    ...
    # kout = kin_array - (kin_array . normal) normal
...
def get_axis(number, resolution):
    ...
    # Create the real space rotation_axis
    ...
    # Create wave number rotation_axis
...
def get_axes_3d(numbers, resolutions):
    ...
    for idx in range(3):
        ...
        holder['real rotation_axis'].update({axis_name[idx]: np.copy(tmp_real)})
        holder['wavevec rotation_axis'].update({axis_name[idx]: np.copy(tmp_wavenum)})

    return holder


def get_fft_mesh_2d(dy, ny, yc, dz, nz, zc):
    # get the y rotation_axis
    ky_list = np.fft.fftshift(np.fft.fftfreq(ny, d=dy) * 2 * np.pi)
    ky_list += yc

    # Get the z rotation_axis
